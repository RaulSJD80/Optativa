...
useEffect(() => {
  const obtenerIncidencias = async () => {
    try{
      ...
      console.log(data);setIncidencias(data);
    } catch (e){
  ...
  const obtenerUsuarios = async () => {
    try{
      ...
      const data = await response.json();
      console.log(data);setUsuarios(data);
    } catch (e){
  ...
  obtenerUsuarios();
      ...
      const agregarIncidencia = async (  nuevo_usuario, nuevo_titulo, nuevo_descripcion, nuevo_categoria, nuevo_nivel_urgencia,
    nuevo_ubicacion ) => {
      try{
  ...
  return (
    <div className = "b" style = {{ backgroundImage : 'url (${Fondo})', backgroundSize: "cover", height: "100vh" }}>
    <Header/>
